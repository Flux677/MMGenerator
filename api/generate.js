...
\`\`\`yaml
Skills:
- skill{s=ON_SPAWN_INIT} @self ~onSpawn
- skill{s=CHECK_PHASE} @self ~onTimer:20
- skill{s=ON_HIT_COUNTER} @self ~onDamaged
- skill{s=ON_ATTACK_COMBO} @target ~onAttack
- skill{s=ON_DEATH_EXPLOSION} @self ~onDeath
- skill{s=ON_TARGET_CHANGE} @target ~onTarget
- skill{s=ON_TELEPORT_EFFECT} @self ~onTeleport
\`\`\`

**Inline Conditions:**
\`\`\`yaml
# Execute only if condition met
- damage{a=20} @target ?varequals{var=caster.powered;val=true}
- summon{t=MINION} @self ?health{h=<50%}

# Cast different skill if condition
- skill{s=AGGRESSIVE_SKILL} @target !?varequals{var=caster.defensive;val=true}
- skill{s=DEFENSIVE_SKILL} @self ?varequals{var=caster.defensive;val=true}
\`\`\`

=== LIBDISGUISES INTEGRATION ===
MANDATORY: Use LibDisguises for visual
\`\`\`yaml
Disguise:
  Type: PLAYER
  Player: SkinName
...
1. ✅ Use variables EXTENSIVELY for state tracking
2. ✅ Implement ALL requested advanced mechanics
3. ✅ ${aiComplexity.toUpperCase()} AI complexity
4. ✅ ${difficulty.toUpperCase()} difficulty balance
5. ✅ LibDisguises for visual (no ModelEngine)
6. ✅ Valid MythicMobs syntax only
7. ✅ Production-ready, tested configurations
8. ✅ Detailed comments explaining mechanics
9. ✅ Boss bar with phases if multi-phase
10. ✅ Complete skill trees with dependencies
